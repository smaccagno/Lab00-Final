...
export default class InvoiceExcelEditor extends NavigationMixin(LightningElement) {
    ...
    getExactValueFromDataset(field, value, caches = null) {
        ...
        switch (field) {
            case 'partner':
                const partner = this.partners.find(
                    p => {
                        ...
                        const cleanedPartnerName = this.cleanValue(p.Name);
            ...
            case 'provincia':
                // Cerca nella lista delle province per trovare il valore esatto (pulisci anche i valori del dataset)
                const provinciaExact = this.provinceList.find(
                    p => {
                        if (!p) return false;
                        const cleanedProvincia = this.cleanValue(p);
                        return cleanedProvincia.toLowerCase() === lowerValue;
                    }
                );
                return provinciaExact ? provinciaExact : trimmedValue;

            case 'regione':
                // Cerca nella lista delle regioni per trovare il valore esatto (pulisci anche i valori del dataset)
                const regioneExact = this.regioniList.find(
                    r => {
                        if (!r) return false;
                        const cleanedRegione = this.cleanValue(r);
                        return cleanedRegione.toLowerCase() === lowerValue;
                    }
                );
    ...
    isValueInDataset(field, value) {
        ...
        // Pulisci il valore da caratteri nascosti prima del confronto
        const cleanedValue = this.cleanValue(value.toString());
    ...
    validateField(row, field, value, caches = null) {
        ...
        switch (field) {
            ...
            case 'noProfitCategory':
                ...
                const cleanedNoProfit = row.noProfit ? this.cleanValue(row.noProfit) : '';
                ...
                const updatedVisits = invoiceGroup.visits.map((visit, visitIndex) => {
                    ...
                    if (useIndexMatching && visitIndex < availableVisitDetails.length) {
                        ...
                        if (candidateByIndex && 
                            ...
                            candidateByIndex.id) {
                            // Se il numero corrisponde esattamente, usa sempre l'indice come matching principale
                            // perché l'ordine dovrebbe essere preservato dal backend
                    ...
                    if (visitResult && visitResult.id) {
                        ...
                        // Se è stato usato il matching per indice, marca anche l'indice come assegnato
                        if (useIndexMatching && visitIndex < availableVisitDetails.length) {
                            assignedVisitIndices.add(visitIndex);
                        }
                        ...
                        console.log(`[updateOrganizedInvoicesWithSaveResults] Visita ${visitIndex} assegnata:`, {
                            ...
                            originalIndex: visitResult.originalIndex
                        });
